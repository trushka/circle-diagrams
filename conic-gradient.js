...
var _ = self.ConicGradient = function(o) {
	...
	this.stops = Array.isArray(stops)?stops:(stops || "").split(/\s*,(?![^(]*\))\s*/); // commas that are not followed by a ) without a ( first
	...
	paint: function() {
		...
		for (var i = 0; i < 360;) {
			if (i/360 + e0 >= stop.pos) {
				...
				} while(stop && stop != prevStop && i/360 + e0 >= stop.pos);
...
_.ColorStop = function(gradient, stop) {
	...
	if (stop && stop.match) {
		...
		this.color = _.ColorStop.colorToRGBA(stop[0]);
		this.pos=(stop[1]||0);
	}
}
